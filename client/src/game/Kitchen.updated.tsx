...
const Kitchen = ({ onExit }: KitchenProps) => {
  ...
  useEffect(() => {
    ...
    console.log("Estado actual - Guía:", isGuideMode);
  }, [refrigeratorFood, pantryFood, isGuideMode]);
  ...
  const handleCook = () => {
    ...
    // Evaluar la calidad de la comida
    let mealQuality = "regular";
    if (totalProtein >= 20 && avgSustainability >= 7) {
      mealQuality = "excelente";
    } else if (totalProtein >= 10 || avgSustainability >= 5) {
      mealQuality = "buena";
    }
    ...
    // Mostrar mensaje según la calidad
    let message = "";
    if (language === 'en') {
          ...
          <Card key={recipe.id} className="bg-[#FFFAF0] border-4 border-[#F5D6A4] shadow-lg rounded-2xl overflow-hidden">
            ...
            <CardContent className="p-4">
              ...
              <div className="mt-2 flex items-center">
                ...
                <span className="ml-2 text-[#7E4E1B] text-sm">{recipe.sustainability}/10</span>
              </div>
